...
//    setupMark();
    ...
    test("Tabs are removed from the addition", 3, parseAndPrint, "7 + 35", "7\t+35");
    test("Newlines are removed from the addition", 3, parseAndPrint, "35 + 7", `
        35  +
        7`);
    test("Parentheses are removed from addition", 3, parseAndPrint, "5 + 4 + 3 + 2 + 1 + 2 + 3 + 4 + 5 + 6 + 7", "5+((4+3)+2) +1+ (2+3 + 4)+5+6+7" );
    test("Multiplication is properly spaced", 3, parseAndPrint, "6 * 7", "6*7");
    test("Multiplication is performed before addition (1)", 3, parseAndPrint, "6 * 6 + 6", "(6*6)+6");
    test("Multiplication is performed before addition (2)", 3, parseAndPrint, "7 + 5 * 7", "7+(5*7)");
    test("Multiplication is performed before addition (3)", 3, parseAndPrint, "6 * (6 + 6)", "6*(6+6)");
    test("Multiplication is performed before addition (4)", 3, parseAndPrint, "(7 + 5) * 7", "(7+5)*7");
  
    test("Complex expressions are supported", 3, parseAndPrint, "7 + 2 * 7 + 3 * 6 + 3", "7+2*7+3*6+3");
    test("Parentheses are correctly supported", 3, parseAndPrint, "(2 + 1) * 7 + 3 * 2 * 3 + 2 * (1 + 0) + 1", "(2+1)*7+(3*2*3)+2*(1+0)+1");
    test("Extra paren are removed", 3, parseAndPrint, "(2 + 1) * 7 + 3 * 2 * 3 + 2 * (1 + 0) + 1", "(2+1)*7+(3*(2)*3)+2*((1+(0)))+1");
    });
...
describe('Testing unary negation', () => {
    ...
    test("Unary minus in addition is properly parenthesized 1", 3, parseAndPrint, "43 + -1", "(43)+-(1)");
    test("Unary minus in addition is properly parenthesized 2", 3, parseAndPrint, "43 + -1", "(43)+(-1)");
    test("Unary minus is supported in subtraction", 3, parseAndPrint, "43 - -1", "43--1");
    test("Unary minus in subtraction is properly parenthesized 1", 3, parseAndPrint, "43 - -1", "(43)--(1)");
    test("Unary minus in subtraction is properly parenthesized 2", 3, parseAndPrint, "43 - -1", "(43)-(-1)");
});
describe('testing variables', () => {
    ...
    test("Complex expressions are properly printed",  3, parseAndPrint, "(x * y + 1) * y * x", "(x*y+1)*y*x");
});